...
bool Planner::_populate_block(block_t * const block, bool split_move,
  ...
  TERN_(HAS_EXTRUDERS, if (de < 0) SBI(dm, E_AXIS));
    ...
    #if TAOLI_SERIAL && LINEAR_AXES >= 4
    uint32_t esteps = ABS(esteps_float) + 0.5f;
    #else
  ...
  #elif ENABLED(MARKFORGED_XY)
    #if ENABLED(DUAL_X_CARRIAGE) 
    if(extruder) block->steps.set(LINEAR_AXIS_LIST(ABS(da - db), ABS(db), ABS(dc), ABS(di), ABS(dj), ABS(dk)));
    else block->steps.set(LINEAR_AXIS_LIST(ABS(da + db), ABS(db), ABS(dc), ABS(di), ABS(dj), ABS(dk)));
    #else
  ...
  if (true LINEAR_AXIS_GANG(
    ...
    switch(fifth_mode) {
    case 0://we联动
      steps_dist_mm.i = steps_dist_mm.e * fifth_e_factor;
    ...
    const float isteps_float = steps_dist_mm.i * settings.axis_steps_per_mm[I_AXIS];
    block->steps.i = ABS(isteps_float) + 0.5f;
  #else
  ...
  float vmax_junction_sqr; // Initial limit on the segment entry velocity (mm/s)^2
    ...
    if (moves_queued && !UNEAR_ZERO(previous_nominal_speed_sqr)) {
      ...
      else vmax_junction = previous_nominal_speed;
...
bool Planner::buffer_line(const xyze_pos_t &cart, const_feedRate_t fr_mm_s, const uint8_t extruder/*=active_extruder*/, const float millimeters/*=0.0*/
  ...
  TERN_(HAS_POSITION_MODIFIERS, apply_modifiers(machine));//执行调平的高度补尝
