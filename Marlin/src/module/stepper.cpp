  ...
  if (!current_block) {
    ...
    if ((current_block = planner.get_current_block())) {
      ...
      while (current_block->flag & BLOCK_MASK_SYNC) {
        ...
        #endif
        #if ENABLED(MARKFORGED_XY) && ENABLED(DUAL_X_CARRIAGE)
        if (!is_sync_fans) _set_position(current_block->position, current_block->extruder);
        #else
      ...
      if(my_print_status == PRINT_RUNNING) {
        ...
        recovery.info.nominal_speed_sqr = current_block->nominal_speed_sqr;
      }
      #else
      ...
      // Flag all moving axes for proper endstop handling
        ...
        #define X_MOVE_TEST (current_block->steps.a != current_block->steps.b) //这里是不是有问题?假如a == b它仍然是动的啊
      #else
